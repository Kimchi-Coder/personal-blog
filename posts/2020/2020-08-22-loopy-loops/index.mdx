...
Like most (if not all?) programming languages, Javascript has iterables. These are things that may be looped or iterated over with for-loops, while-loops, .map(), .forEach(), etc. Strings, arrays, and maps are iterables in JS, however, objects are not!  Take, for example, the following:
